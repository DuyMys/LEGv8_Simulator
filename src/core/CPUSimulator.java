...
public class CPUSimulator {
    ...
    public CPUSimulator(InstructionConfigLoader configLoader) {
        ...
        this.microStepManager = new MicroStepManager(this, registerFile, alu, controlUnit, memory, this::updateFlags);
    ...
    public void step() {
        ...
        if (microStepManager.isEmpty()) { // This now checks if we've gone past the end
            ...
            if (!branchTaken) {
                pc++; // Nếu không nhảy, đi tới lệnh tiếp theo
            } else {
                // Nếu nhảy, PC đã được cập nhật bởi một micro-step rồi,
                // nên không cần làm gì ở đây.
                // Reset lại cờ branchTaken cho lệnh tiếp theo.
                this.branchTaken = false; 
            }
            printState();
        }
